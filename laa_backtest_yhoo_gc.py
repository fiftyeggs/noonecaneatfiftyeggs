#golden cross v1: use golden cross to increas equities exposure during good times
#idea: constrain portfolio to require 33% (or whatever) in VTI

#TODO: less aggresive bull market filter? e.g. large equity exposure. rest: uncorrelated exposure. cash? bonds?
...
def backtest(in_lookback, N, w1, w2, w3):
    ...
    for idx in range(max(mvp_lookback+1,long_ma+2),dayCount):

        #calculate 50 ma, and 200 ma, also for previous day so we can find crossovers
        small_ma_values = rolling_mean(allClose[equity_symbol].ix[dates[idx-2-small_ma:idx-1]],small_ma)
        long_ma_values = rolling_mean(allClose[equity_symbol].ix[dates[idx-2-long_ma:idx-1]],long_ma)

        #pdb.set_trace()

        #init bull_market flag if it has never been set
        if bull_market == None:
            if small_ma_values.ix[-1]>long_ma_values.ix[-1]:
                bull_market = True
            else:
                bull_market = False

        #if golden cross has occured
        if small_ma_values.ix[-2]<long_ma_values.ix[-2] and small_ma_values.ix[-1]>long_ma_values.ix[-1]:
            #set bull market flag
            bull_market = True
        #if death cross has occured
        elif small_ma_values.ix[-2]>long_ma_values.ix[-2] and small_ma_values.ix[-1]<long_ma_values.ix[-1]:
            #unset bull market flag
            bull_market = False
        
        

        #if dates[idx].month==5 and dates[idx].year==2003 and dates[idx].day > 28: #debug statement
        #   pdb.set_trace()
        ...
        if (dates[idx]==dates[-1]) or (dates[idx]!=dates[-1] and dates[idx].month != dates[idx+1].month): #CASES: 1) last date, also last day of month (assuming we run backtest past midnight) THE DATES MUST BE MOST UP TO DATE PRICE DATA, and must be updated right before backtest (or else if price data is old and this executes on the 1st, it will NOT work), 2) not last day of data, today's month different from tomorrow's month. 
            ...
            #add 'cash' security to cum returns so we can calculate average returns with cash
            if month_turn_flag:
                cash_cum_returns = Series([1 for x in cum_returns.index],index=cum_returns.index)
                cum_returns_dict['CASH'] = cash_cum_returns
            ...
            if len(tradable_symbols)>1:
                ...
                for symbol in tradable_symbols:
                    weights[symbol] = 0
                    li_dict[symbol] = momentum_ranking[symbol]["rank"]*momentum_weight + variance_ranking[symbol]["rank"]*variance_weight + correlation_ranking[symbol]["rank"]*correlation_weight #THE RANKING: how we end up picking the asset classes
                #select lowest (top) Li stocks

                #BULL MARKET CONDITION: right now, if we're in a bull market by gold cross, invest in equities
                
                if not bull_market:
                    li_sorted_all = sorted(li_dict.items(), key=lambda x: x[1])
                if bull_market:
                    #trading = [equity_symbol,bond_symbol]
                    trading = [equity_symbol]
                    li_sorted_all = sorted(li_dict.items(), key=lambda x: x[1] if not x[0] in trading else 0)
                    #pdb.set_trace()
                
                #NON BULL MARKET CONDITION VERSION
                '''
                li_sorted_all = sorted(li_dict.items(), key=lambda x: x[1])
                '''


                li_sorted = li_sorted_all[0:top]


                #pdb.set_trace()

                #ABSOLUTE MOMENTUM: make sure momentum isn't negative. if it is, set weight to 0 (cash).
                for li in li_sorted:
                    symbol = li[0]
                    if (momentum_ranking[symbol]["value"]-1)>0:
                        weights[symbol] = 1.0/float(top)
                        #pdb.set_trace()


                    #AGGRESSIVE BULL MARKET CONDITION: all in equities when it's a bull market
                    
                    if bull_market:
                        if symbol==equity_symbol:
                            weights[symbol] = 1.0
                            #weights[symbol] = float(top-1)/float(top)
                            #pdb.set_trace()
                        else:
                            weights[symbol] = 0.0
                    

                    #SEMI-AGGRESSIVE BULL MARKET CONDITION: 66% in equities, 33% in bonds, when it's a bull market
                    '''
                    if bull_market:
                        if symbol==equity_symbol:
                            #weights[symbol] = 1.0
            ...
            if month_turn_flag:
                ...
                weights_sum  = weights_sum+weights[first_symbol]
                ...
                for symbol in tradable_symbols:
                    ...
                    weights_sum  = weights_sum+weights[symbol]
                
                #CASH LOGIC: if weights don't sum to 1 (we have cash), add weighted average of 0% return (1.0)
                if weights_sum < 1:
                    num_cash_positions = round((1-weights_sum)/(1.0/float(top)),5)
                    average_returns += cum_returns_dict['CASH']*(num_cash_positions/float(top))
                    #pdb.set_trace()
